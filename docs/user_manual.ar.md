...
### الإصدار: `v1.8.0` — المعمارية السيادية وسرب الوكلاء المتعددين ومحرك المستندات الفاخرة
...
4. [مرجع أوامر سطر الأوامر الشامل (CLI Reference)](#4-مرجع-أوامر-سطر-الأوامر-الشامل-cli-reference)
   ...
   - [محرك تصدير المستندات الفاخرة (4 Themes PDF Engine)](#7-محرك-تصدير-المستندات-الفاخرة-pdf--html-export)
   - [تنسيق سرب الوكلاء المتوازي (Parallel Swarm Dispatcher)](#8-تنسيق-سرب-الوكلاء-المتوازي-وفض-النزاعات-dispatch)
...
- **`@tidy/office` (حزمة الأعمال المكتبية)**: حزمة مستقلة تماماً وقابلة للفصل، تضيف قدرات إدارة العملاء (CRM)، الفواتير المحسوبة آلياً، عروض الأسعار التجارية (Proposals)، كتالوج المنتجات والخدمات (Products)، إدارة المصروفات، جدول المواعيد، وتوليد ملفات العملاء الاستخباراتية (AI Dossier) مع محرك تصدير مستندات PDF فاخر بـ 4 ثيمات (`modern`, `minimal`, `luxury`, `corporate`).
...
│  - 4-Theme PDF Engine       - PocketOffice Migration Bridge           │
└───────────────────────────────────┬────────────────────────────────────┘
                                    │
                                    ▼
┌────────────────────────────────────────────────────────────────────────┐
...
│            40 MCP Tools • Parallel Swarm • Conflict Adjudicator        │
└────────────────────────────────────────────────────────────────────────┘
```
...
#### 7. عروض الأسعار التجارية وإدارتها (`proposal`)
إدارة دورة حياة عروض الأسعار الاحترافية B2B وربطها بالعملاء وبنود العمل وجداول الدفعات:
```bash
# استعراض قائمة عروض الأسعار
node bin/tidy.js proposal list

# إنشاء عرض سعر جديد لعميل
node bin/tidy.js proposal create --client cli_xxxx --title "تطوير معمارية الذكاء الاصطناعي السيادية" --currency USD
```

#### 8. كتالوج المنتجات والخدمات (`product`)
إدارة قائمة المنتجات والخدمات والأسعار المعتمدة للفوترة وعروض الأسعار:
```bash
# استعراض قائمة المنتجات المتاحة
node bin/tidy.js product list

# إضافة أو تحديث منتج/خدمة
node bin/tidy.js product upsert --sku "SKILL-CORE" --name "حزمة المهارات الأساسية" --price 499 --unit "license"
```

#### 9. محرك تصدير المستندات الفاخرة (PDF & HTML Export)
تصدير فوري للفواتير وعروض الأسعار وملفات العملاء بـ 4 تصاميم راقية دون الحاجة لمتصفح:
```bash
# تصدير عرض سعر أو فاتورة إلى PDF بالثيم الفاخر
node bin/tidy.js export proposal prop_xxxx --theme luxury --format pdf

# خيارات الثيمات الأربعة:
# --theme modern     (تصميم تقني معاصر بخطوط هندسية)
# --theme minimal    (تصميم سويسري فائق البساطة والتركيز)
# --theme luxury     (تصميم راقٍ بلمسات فخمة وتدرجات راقية)
# --theme corporate  (تصميم مؤسسي رسمي معتمد للشركات)
```

#### 10. تنسيق سرب الوكلاء المتوازي وفض النزاعات (`dispatch`)
تشغيل وتوزيع المهام المعقدة بالتوازي عبر وكلاء متخصصين مع فض النزاعات آلياً:
```bash
# توزيع مهام التدقيق وإعادة الهيكلة بالتوازي بين الوكيل المحلي ومساعد الكود
node bin/tidy.js dispatch --tasks "audit,refactor" --agents "qwen,coder"
```
...
### الأدوات المتاحة لوكلاء الذكاء الاصطناعي (40 أداة متخصصة عبر 7 تصنيفات وظيفية):
...
#### 5. المهام وسرب الوكلاء المتعددين وطبقة التحكم (Tasks, Control Plane & Parallel Swarm)
| اسم الأداة | الغرض والوصف |
| :--- | :--- |
...
| `tidy_parallel_dispatch`| تشغيل وتوزيع المهام بالتوازي عبر وكلاء متعددين في صناديق رملية معزولة مع فض النزاعات آلياً. |
| `tidy_context_compile` | تجميع سياق منضبط تحت سقف التوكنات عبر 5 طبقات للبرومبت دون تسريب بيانات. |
| `tidy_intent_route` | تحليل نوايا التعليمات وترشيح المهارات مع فرض سقف صارم 2-3 مهارات كحد أقصى. |
| `tidy_taxonomy_classify`| تصنيف الذاكرة وقرارات الكود ضمن تاكسونومي الذاكرة الـ 8 المعياري. |

#### 6. حزمة إدارة الأعمال والمكتب التجاري (`@tidy/office`)
| اسم الأداة | الغرض والوصف |
| :--- | :--- |
...
#### 7. عروض الأسعار والمنتجات ومحرك تصدير مستندات PDF (Proposals, Products & PDF Export)
| اسم الأداة | الغرض والوصف |
| :--- | :--- |
