...
- **`@tidy/core` (`packages/core`)**: نواة المنظومة المعتمدة على `node:sqlite` المدمج بدون أي مجمّعات C++، توفر نقطة توسعة المخططات `registerSchema()`، وفهرسة FTS5 ثنائية اللغة، ونموذج الحلقات الثلاث، ومحرك فحص أدوات الذكاء الاصطناعي المتعددة (`multi-tool-scanner.js`)، ومُدقق الامتثال لقواعد Skills-LAB الـ 15 (`skills-validator.js`)، ومولد القوالب المبتدئة (`boilerplate-generator.js`)، ومجموعات الاستوديو (`studio.js`)، ومستخرج الذاكرة وحصاد المعارف (`knowledge-harvester.js`)، ومحرك اكتشاف المهارات المجتمعية (`skills-loader.js`)، ومولّد موجزات المهام ذاتية الاكتفاء (`brief-generator.js`)، والوكلاء والتطبيقات المصغرة (Tasks, Snippets, Journal, Vault).
...
- **`@tidy/mcp` (`packages/mcp`)**: خادم Stdio JSON-RPC 2.0 MCP المتكامل (20 أداة ذكية + 6 موارد حية تشمل الذاكرة والمجالات والمهام وإعدادات الحوكمة والتدفق المالي اللحظي).
- **`@tidy/skill` (`packages/skill`)**: مهارة TidyFactor الرسمية المعتمدة وفق القواعد الـ 15 وميثاق CDL v2.0.
- **`@tidy/desktop` (`apps/desktop`)**: تطبيق سطح المكتب بنظام ويندوز مبني بتقنية Electron بأمان `contextIsolation: true` وشاشة HUD طافية مستدعاة بالاختصار العالمي `Alt+Space` مع تبويبات مخصصة لاستوديو المهارات والوكلاء، واستوديو استخراج الذاكرة، والـ CRM، والفواتير والتدفق النقدي.
- **`apps/web`**: واجهة ويب لإدارة المنظومة عبر المتصفح على المنفذ `3840` مع واجهات برمجية كاملة واستوديو المهارات والوكلاء واستوديو استخراج الذاكرة.
...
3. **استوديو المهارات والوكلاء الشامل (Universal Skills & Agents Studio)**: اكتشاف فوري للمهارات والوكلاء عبر Claude Code وCursor وCodex وWindsurf وCopilot وAntigravity وAmp وAider؛ محرر نصوص برمجية مونو-سبيس متكامل مع حفظ لحظي وتزامن YAML Frontmatter؛ تنظيم المجموعات والمفضلات غير التدميري على SQLite؛ وتدقيق فوري للامتثال لقواعد التوثيق.
4. **استوديو استخراج الذاكرة وحصاد المعارف (Knowledge Harvester Studio)**: فحص آلي لبيئات الوكلاء وعقول الجلسات عبر واجهة مستقلة ثنائية اللوحات؛ معاينة متدفقة حية لمحتوى المستند الكامل بتنسيق Markdown؛ تعديل فوري للطبقة والتصنيف؛ ربط تكاملي مباشر ومتبادل مع استوديو المهارات عبر زر "فتح في الاستوديو"؛ وإدخال ذري مع تحديث فهارس FTS5 ومنع التكرار.
5. **مركز المهارات والوكلاء الفرعيين (Skills & Sub-Agents Hub)**: أدوار مدمجة جاهزة (`planner`, `coder`, `researcher`, `scribe`)، بالإضافة إلى دمج فوري وتلقائي لكافة مهارات مجتمع TidyFactor كوكلاء فرعيين مباشرين.
6. **توليد موجزات المهام المستقلة (Self-Contained Task Briefs)**: تركيب موجز مهام ماركداون متكامل يجمع الهدف، وسياق الحلقات الثلاث، وقواعد المهارة، ومصفوفة التحقق الذاتي.
7. **جناح إدارة الأعمال السيادي (Sovereign Business Suite)**: تتبع الصفقات، إصدار الفواتير الرسمية وحساب الضرائب والخصومات، كشف التدفقات النقدية اللحظي، وتجميع تقارير العملاء الشاملة.
8. **تعدد واجهات الاستخدام**: سطر الأوامر (CLI)، خادم MCP للوكلاء، تطبيق سطح المكتب (Electron مع شاشة HUD باختصار `Alt+Space`)، وواجهة الويب.

---

## 🖥️ استوديو إدارة سطح المكتب الرائد (`@tidy/desktop`)

يوفر نظام **تايدي** واجهة سطح مكتب تنفيذية أصلية لنظام ويندوز مبنية بواسطة **Electron**، صُممت لتكون لوحة القيادة البصرية المباشرة المتصلة بقاعدة بيانات SQLite المحلية (`~/.tidy/tidy.db`) مع شاشة طافية ذكية فائقة السرعة.

<div align="center" dir="rtl">

### 📸 استعراض واجهات تطبيق سطح المكتب (Tidy Studio Showcase)
...
### 🌟 تبويبات تطبيق سطح المكتب (14 تبويباً متكاملاً)
1. **لوحة التحكم (Overview)**: الرؤية الشاملة لكافة مؤشرات النظام والمهام المعلقة.
2. **استوديو المهارات والوكلاء (Skills Studio)**: محرر أكواد ومفحوص لامتثال القواعد الـ 15.
3. **مستكشف الذاكرة (Memory Explorer)**: واجهة مرئية لبحث نصوص FTS5 وتقييم الأهمية.
4. **حصاد المعارف (Knowledge Harvester)**: استكشاف ملفات الوكلاء وعقول الجلسات وحفظها ذرياً.
5. **مشغّل الوكلاء (Agent Runner)**: إطلاق الوكلاء وتوليد موجزات المهام الذاتية.
6. **استوديو MCP (MCP Studio)**: فهرس كامل لـ 29 أداة و9 موارد حية وفاحص JSON-RPC.
7. **لوحة كانبان المهام (Tasks Board)**: تصنيف وتوزيع المهام على الوكلاء.
8. **خزينة القصاصات (Code Snippets)**: حفظ الأكواد مع التمييز اللوني والنسخ السريع.
9. **اليوميات التقنية (Daily Journal)**: تدوين الملاحظات البرمجية ومتابعة الإنجاز اليومي.
10. **الخزينة المشفرة (Encrypted Vault)**: حفظ الأسرار والمفاتيح دون تسريب سحابي.
11. **إدارة العملاء (CRM Pipeline)**: إدارة مسارات الصفقات وقوائم التواصل.
12. **الفواتير والتحصيل (Invoices & Billing)**: فوترة مجزأة وحساب الضرائب وحالة السداد.
13. **التدفق المالي (Cashflow Statement)**: كشف الأرباح والمصروفات وهامش الربحية.
14. **الإعدادات وصحة النظام (Settings & Health)**: صيانة SQLite وضغط الذاكرة وتبديل سياق العمل.

### ⚡ شاشة المساعد الطافية العالمية (`Alt + Space`)
استدعِ مساعدك الذكي في أي لحظة ومن أي شاشة دون مغادرة محررك البرمجي عبر الاختصار `Alt + Space` لالتقاط القرارات، البحث في الذاكرة، أو تفويض المهام.

```bash
# تشغيل تطبيق سطح المكتب في بيئة التطوير
npm run desktop

# بناء حزمة التثبيت الأصلية لويندوز (.exe)
